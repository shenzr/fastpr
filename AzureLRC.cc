...
AzureLRC::AzureLRC(vector<int> param) {
    _n = param[0];
    _k = param[1];
    _l = param[2];
    _g = param[3];
    ...
    string enc_file_name = string("metadata/azurelrcEncMat_") + to_string(_n) + string("_") + to_string(_k) + "_" + to_string(_l) + "_" + to_string(_g); 
...
vector<int> AzureLRC::getDecodeCoef(vector<int> from, int to) {
//    cout << "from: ";
...
//    cout << "to: " << to << endl;
    ...
    if (local) {
        ...
        int* enc_matrix = (int*)malloc(sizeof(int)*group.size()*_k);
        memset(enc_matrix, 0, sizeof(int)*group.size()*_k);

        for (int i=0; i<_k; i++)
            enc_matrix[i*_k+i] = 1;
        for (int i=0; i<_g; i++)
            memcpy(enc_matrix + (_k+i) * _k,
                    _complete_matrix + (_k+_l+i) * _k,
                    sizeof(int) * _k);
