...
CC_FILES = $(filter-out $(SRC_DIR)/FastPRCoordinator.cc $(SRC_DIR)/FastPRPeerNode.cc $(SRC_DIR)/FastPRHotStandby.cc $(SRC_DIR)/AzureLRCCoordinator.cc $(SRC_DIR)/AzureLRCPlusTest.cc, \
		   $(wildcard $(SRC_DIR)/*.cc))
...
all : directories FastPRCoordinator FastPRPeerNode FastPRHotStandby AzureLRCCoordinator
...
FastPRCoordinator : $(SRC_DIR)/FastPRCoordinator.cc $(O_FILES)
	$(CC) $(CFLAGS) -o $@ $^ $(CLIBS)
...
AzureLRCCoordinator : $(SRC_DIR)/AzureLRCCoordinator.cc $(O_FILES)
	$(CC) $(CFLAGS) -o $@ $^ $(CLIBS)
...
clean :
	rm -f FastPRCoordinator FastPRPeerNode FastPRHotStandby AzureLRCCoordinator obj/*
